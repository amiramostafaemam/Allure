    ...
    try {
      ...
      for(const line of parsed.data.items){
        ...
        lines.push({
            ...
            unitPricePounds:prod.pricePounds*100,
        });
      }
      if(totalPounds<50){
        res.status(400).json({error:"Total below polar minimum (e.g. Total requires at least £50)"});
        return;
      }
      ...
      const checkout=await polarCreateCheckout(env,{products:[env.POLAR_CHECKOUT_PRODUCT_ID],
        prices:{[env.POLAR_CHECKOUT_PRODUCT_ID]:[{amount_type:"fixed",price_amount:totalPounds,price_currency:"EGP"}]},
        success_url:successUrl,return_url:returnUrl,
    external_customer_id:userId,metadata:{checkout_session_id:session.id}});
